...
```{r}
drop_cols <- c("Alley", "Street" ,"Utilities" , 
...
Following colums are log adjusted

```{r}
log_cols <- c("LotFrontage", "LotArea", "MasVnrArea")
...
```{r}
clean_df <- one_hot_encode(clean_df, "LandContour")
```
...
clean_df <- one_hot_encode(clean_df,"LandSlope")
clean_df <- one_hot_encode(clean_df,"Neighborhood")


clean_df_1 <- clean_df %>% mutate(Condition2 = NULL) %>% one_hot_encode("Condition1") %>% select(contains("Condition1")) %>% mutate(Condition1_RRNn = 1-rowSums(.)) %>% select(-contains("RRne"))
clean_df_2 <- clean_df %>% mutate(Condition1 = NULL, Condition1 = Condition2) %>% one_hot_encode("Condition1") %>% select(contains("Condition1")) %>% mutate(Condition1_RRNn = 1-rowSums(.))
clean_df = clean_df %>% cbind( (clean_df_1 + clean_df_2) %>% select(-contains("Norm"))) %>% select(-Condition1, -Condition2)
clean_df <- one_hot_encode(clean_df,"BldgType")
clean_df <- one_hot_encode(clean_df,"HouseStyle")
```

For condition variables, we are enconding as the variable is present or not. We remove normal from both the variables as it is not adding any value. We are also removing RRNe as it has very few observations
